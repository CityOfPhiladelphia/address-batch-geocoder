...
def process_csv(config_path) -> pl.LazyFrame:
    ...
    try:
        ...
        # 1. We reinrich with AIS to see if the new TomTom parsed address is
        # searchable with AIS, allowing us to potentially recover enrichment fields
        # 2. That either geocodes or doesn't. We take the records that AIS failed to geocode.
        # 3. We use the tomtom matched record for the records that AIS failed to geocode.
        # 4. We rejoin those to the records that AIS did manage to reinrich
        # 5. We rejoin that again back to the original 'has_geo' -- the records that never needed
        # to be matched to TomTom in the first place.

        ais_reinriched = enrich_with_ais(config, tomtom_enriched, uses_full_address, ais_enrichment_fields)
        ...
        reinriched_rejoined = pl.concat([reinriched_has_geo, tomtom_fallback], how="diagonal").sort("__geocode_idx__")

        rejoined = (
            pl.concat([has_geo, reinriched_rejoined])
